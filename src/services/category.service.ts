...
export async function findCategories(): Promise<Array<{ title: string, url: string; category: string }>> {
    const categoryHTML = await fetchInfo();
    const dom = new JSDOM(categoryHTML);

    // https://www.vedomosti.ru/rss/rubric/...
    const anchors = Array.from(dom.window.document.querySelectorAll(`a[href^='${VEDOMOSTI_BASE_URL}/rss/rubric/']`))
